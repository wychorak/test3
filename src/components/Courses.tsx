...
const categories = [
  {
    ...
    price: '2 500 PLN',
    icon: <Bike className="w-10 h-10 md:w-12 md:h-12 text-[#a855f7] mb-4 md:mb-6" />,
  ...
  },
  {
    ...
    priceAuto: '3 400 PLN',
    icon: <Car className="w-10 h-10 md:w-12 md:h-12 text-[#a855f7] mb-4 md:mb-6" />,
  ...
  },
  {
    ...
    price: '4 500 PLN',
    icon: <Truck className="w-10 h-10 md:w-12 md:h-12 text-[#a855f7] mb-4 md:mb-6" />,
...
export function Courses() {
  ...
  return (
    <section id="courses" className="py-20 md:py-32 relative bg-black">
          ...
          {categories.map((course, index) => (
            <motion.div
              ...
              className={`relative group flex flex-col h-full ${
                course.popular ? 'md:scale-105 z-10' : ''
              }`}
            >
              <SpotlightCard className={`h-full p-8 md:p-10 flex flex-col ${course.bgClass} ${course.popular ? 'shadow-[0_0_40px_rgba(168,85,247,0.2)] border-[#a855f7]/50' : ''}`}>
                <div className="h-48 -mx-8 -mt-8 md:-mx-10 md:-mt-10 mb-8 relative overflow-hidden">
                  ...
                  <div className={`absolute inset-0 bg-gradient-to-t ${course.bgClass === 'bg-white' ? 'from-white' : 'from-[#1d1d1f]'} to-transparent`}></div>
                </div>
                {course.popular && (
                  <div className="absolute top-4 right-4 bg-[#a855f7] text-white text-[10px] md:text-xs font-bold px-3 py-1 md:px-4 md:py-1.5 rounded-full tracking-wide whitespace-nowrap z-20 shadow-[0_0_15px_rgba(168,85,247,0.5)]">
                    NAJPOPULARNIEJSZA
                  </div>
                )}
                ...
                <div className="relative z-10 flex flex-col h-full">
                  ...
                  <p className={`mb-6 md:mb-8 text-sm md:text-base ${course.popular ? 'text-gray-600' : 'text-[#86868b]'}`}>{course.subtitle}</p>
                      ...
                      <div className="flex bg-gray-100 p-1 rounded-xl mb-4 w-full">
                        <button
                          ...
                          className={`flex-1 py-2 text-xs md:text-sm font-semibold rounded-lg transition-all ${
                            transmission === 'manual' ? 'bg-white text-black shadow-sm' : 'text-gray-500 hover:text-black'
                          }`}
                        >
                          Manual
                        </button>
                        <button
                          ...
                          className={`flex-1 py-2 text-xs md:text-sm font-semibold rounded-lg transition-all ${
                            transmission === 'automatic' ? 'bg-white text-black shadow-sm' : 'text-gray-500 hover:text-black'
                          }`}
                        >
                          Automat
                        </button>
                  ...
                  <ul className="space-y-3 md:space-y-4 mb-8 md:mb-10 flex-grow">
                    {course.features.map((feature, i) => (
                      <li key={i} className={`flex items-center text-sm md:text-base ${course.popular ? 'text-gray-800' : 'text-[#f5f5f7]'}`}>
                  ...
                  <button
                    ...
                    className={`block w-full text-center py-3 md:py-4 rounded-full font-semibold text-sm md:text-base transition-colors duration-300 ${
                      course.popular
                        ...
                        : 'bg-white/10 text-white hover:bg-[#a855f7]'
                    }`}
                  >
                    Wybierz {course.name}
                  </button>
